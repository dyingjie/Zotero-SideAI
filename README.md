...
- 当前测试已覆盖空文本字段和异常 HTML note 文本的预览清洗行为
- 当前发送前会对超长上下文做统一截断，避免预览和请求文本无限增长
- 当前测试已覆盖错误 `baseURL` 的失败路径，非法地址会返回可理解错误
- 当前测试已覆盖错误 API Key 的认证失败路径，并返回 401 错误提示
- 当前测试已覆盖网络超时路径，并统一返回 `Request timed out.`
